<x-app-layout>
    <div class="container px-10 mx-auto mt-10">
        <div class="bg-white border border-gray-300 rounded-xl shadow-lg p-5">
            ...
            <table class="min-w-full rounded-xl bg-white border border-gray-300 overflow-hidden display"
                ...
                <thead>
                    <tr>
                        <th class="px-4 py-2 bg-slate-100 text-start border">Dimensi</th>
                        <th class="px-4 py-2 w-[50%] bg-slate-100 text-start border">Sub Dimensi</th>
                    ...
                    @foreach ($subdimensis as $subdimensi)
                        <tr>
                            ...
                            <td class="border px-4 py-2">
                                ...
                                {{-- Delete Form Start --}}
                                <form id="delete-form-{{ $subdimensi->id }}"
                                    action="{{ route('admin.subdimensi.destroy', $subdimensi->id) }}" method="POST"
                                    class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="px-4 py-2 bg-red-600 text-white rounded"
                                        onclick="openModal('{{ route('admin.subdimensi.destroy', $subdimensi->id) }}')">
                                        Hapus
                                    </button>
                                </form>
                                {{-- Delete Form End --}}
                            ...
                            <div class="flex items-center justify-center min-h-screen">
                                ...
                                <div class="bg-white rounded-lg shadow-lg p-6 z-10">
                                    ...
                                    <div class="flex justify-end mt-4">
                                        ...
                                        <form id="delete-form-{{ $subdimensi->id }}"
                                            action="{{ route('admin.subdimensi.destroy', $subdimensi->id) }}"
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if ($.fn.DataTable.isDataTable('#subdimensiTable')) {
                $('#subdimensiTable').DataTable().destroy();
            }

            // Inisialisasi DataTable kembali
            $('#subdimensiTable').DataTable({
                paging: true,
