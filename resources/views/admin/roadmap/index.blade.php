<x-app-layout>
    <div class="container px-10 mx-auto mt-10">
        <div class="bg-white border border-gray-300 rounded-xl shadow-lg p-5">
            <h1 class="text-2xl font-bold mb-4">Daftar Road Map</h1>

            <a href="{{ route('admin.roadmap.create') }}"
                class="bg-blue-600 text-white px-4 py-2 rounded mb-4 inline-block">Tambah Road Map</a>
            ...
            <table class="min-w-full rounded-xl bg-white border border-gray-300 overflow-hidden display"
                ...
                <tbody>
                    @foreach ($roadmaps as $roadmap)
                        <tr>
                            <td class="border px-4 py-2">{{ $roadmap->judul }}</td>
                            ...
                            <td class="border px-4 py-2">
                                ...
                                <form id="delete-form-{{ $roadmap->id }}"
                                    action="{{ route('admin.roadmap.destroy', $roadmap->id) }}" method="POST"
                                    class="inline-block">
                                    @csrf
                                    @method('DELETE')
                                    <button type="button" class="px-4 py-2 bg-red-600 text-white rounded"
                                        onclick="openModal('{{ route('admin.roadmap.destroy', $roadmap->id) }}')">
                                        Hapus
                                    </button>
                                </form>
                                {{-- Delete Form End --}}
    ...
    <script>
        ...
        $(document).ready(function() {
            ...
            if ($.fn.DataTable.isDataTable('#roadmapTable')) {
                $('#roadmapTable').DataTable().destroy();
            }
